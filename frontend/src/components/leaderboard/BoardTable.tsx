...
const LeaderboardRow = memo(function LeaderboardRow({ entry, isCurrentUser }: LeaderboardRowProps) {
  ...
  const getRankBadge = (r: number) => {
    if (r === 1) return <Crown className="w-4 h-4 text-yellow-400 fill-yellow-400" />;
    if (r === 2) return <Award className="w-4 h-4 text-slate-300 fill-slate-300" />;
    if (r === 3) return <Award className="w-4 h-4 text-amber-600 fill-amber-600" />;
    return <span className="text-xs font-mono text-slate-500 w-4 text-center">{r}</span>;
  };

  const getRankBg = (r: number) => {
    if (r === 1) return 'bg-yellow-500/5 border-yellow-500/20';
    if (r === 2) return 'bg-slate-300/5 border-slate-300/10';
    if (r === 3) return 'bg-amber-600/5 border-amber-600/10';
